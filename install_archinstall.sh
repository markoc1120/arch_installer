...
total_size=$(lsblk -b -o SIZE $hd | grep -v SIZE | awk '{print $1}' | head -n 1)
sed -i "s#"%totalsize"#$total_size#g" /tmp/config.json
...
arch_install=$(archinstall --silent --config /tmp/config.json --creds /tmp/creds.json)
